...
class AndroidContinuousListener:
    """
    Continu herstartende Android SpeechRecognizer voor ChemieApp.

    Deze versie:
    - gebruikt de Android UI-thread;
    - gebruikt de basale Google-herkenning;
    - start na onResults of onError opnieuw zonder cancel();
    - bevat logging voor ADB/logcat;
    - voorkomt dubbele geplande herstarts.
    """
    ...
    def _laad_android_klassen(self):
        try:
            ...
            self._fout(
                ...
                f"worden geladen: {type(fout).__name__}: {fout}"
            )
    ...
    def _maak_en_start(self, *args):
        ...
        if self.luistert:
            print(
                "[ANDROID LISTENER]: "
                "START OVERGESLAGEN, MICROFOON LUISTERT AL"
            )
            return
        ...
        try:
            ...
            # Java String gebruiken om de [C-waarschuwing op
            # de oudere Samsung-tablet te voorkomen.
            taal = self.JavaString(self.language)
    ...
    def _maak_listener(self):
        try:
            ...
            self._fout(
                ...
                f"aangemaakt: {type(fout).__name__}: {fout}"
            )

    def _markeer_callback(self, naam):
        print(
            f"[ANDROID CALLBACK]: {naam}"
        )
    ...
    def _verwerk_bundle(
        ...
        try:
            ...
            if resultaten is None:
                print(
                    "[ANDROID LISTENER]: "
                    "GEEN RESULTATENLIJST ONTVANGEN"
                )
                if definitief:
            ...
            if resultaten.size() == 0:
                print(
                    "[ANDROID LISTENER]: "
                    "LEGE RESULTATENLIJST ONTVANGEN"
                )
                if definitief:
                    self._plan_herstart(1.0)
                return
            ...
            if definitief:
                ...
                print(
                    "[ANDROID LISTENER]: "
                    "NIEUWE LUISTERSESSIE PLANNEN "
                    "NA RESULTAAT"
                )

                self._plan_herstart(1.5)

        except Exception as fout:
            self._fout(
                "Spraakresultaat kon niet worden verwerkt: "
                f"{type(fout).__name__}: {fout}"
            )
            self._plan_herstart(2.0)
    ...
    def _verwerk_fout(self, error_code):
        ...
        if code in (6, 7):
            ...
            self._status(
                "SPRAAKHERKENNER BEZET - OPNIEUW PROBEREN"
            )
            wachttijd = 2.0
        ...
        print(
            "[ANDROID LISTENER]: "
            f"NIEUWE LUISTERSESSIE OVER {wachttijd} SECONDEN"
        )
    ...
    def _plan_herstart(self, wachttijd=1.0):
        if not self.actief:
            return
    ...
    def _herstart(self, dt=None):
        ...
        print(
            "[ANDROID LISTENER]: "
            "NIEUWE LUISTERSESSIE STARTEN"
        )

        # Geen cancel() gebruiken. De vorige sessie is al
        # beëindigd via onResults() of onError().
        self._maak_en_start()
    ...
    def _pauzeer_op_ui_thread(self):
        try:
            ...
            print(
                "Microfoon pauzeren gaf melding: "
                f"{type(fout).__name__}: {fout}"
            )
