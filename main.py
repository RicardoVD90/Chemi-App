...
class ChemieApp(App):
    def build(self):
        ...
        self.android_tts_listener = None
        self.android_tts_gereed = False
        self.android_tts_initialiseren = False
        self.android_tts_bezig = False
        self.wachtende_tts_teksten = []
        ...
        self.btn_lab = Button(
            text="LABORATORIUM",
            background_color=get_color_from_hex(KLEUR_TEKST_DONKER),
            **btn_style
        )
    ...
    @run_on_ui_thread
    def initialiseer_android_tts(self, dt=None):
        if platform != "android":
            return
        if self.android_tts_gereed or self.android_tts_initialiseren:
            return
        if autoclass is None or java_method is None:
            print("[ANDROID TTS FOUT]: PyJNIus is niet beschikbaar")
            return

        self.android_tts_initialiseren = True

        try:
            activiteit = autoclass("org.kivy.android.PythonActivity").mActivity
            ...
            class TTSInitListener(PythonJavaClass):
                ...
                @java_method("(I)V")
                def onInit(listener_self, status):
                    Clock.schedule_once(
                        lambda tijd, resultaat=int(status):
                            eigenaar.android_tts_gestart(resultaat),
                        0
                    )

            self.android_tts_listener = TTSInitListener()
            self.android_tts = TextToSpeech(
                activiteit,
                self.android_tts_listener
            )
            print("[ANDROID TTS]: INITIALISATIE GESTART")
    ...
    def android_tts_gestart(self, status):
        self.android_tts_initialiseren = False

        if status != 0:
            self.android_tts_gereed = False
            print(f"[ANDROID TTS FOUT]: INITIALISATIESTATUS {status}")
            return

        try:
            Locale = autoclass("java.util.Locale")
            nederlands = Locale("nl", "NL")
            taalstatus = self.android_tts.setLanguage(nederlands)
            self.android_tts.setSpeechRate(0.9)
            self.android_tts.setPitch(1.0)
            self.android_tts_gereed = True
            print(f"[ANDROID TTS]: GEREED TAALSTATUS={taalstatus}")

            wachtende_teksten = list(self.wachtende_tts_teksten)
            self.wachtende_tts_teksten.clear()
            for tekst in wachtende_teksten:
    ...
    @run_on_ui_thread
    def spreek_android_tekst(self, tekst):
        if not self.android_tts_gereed or self.android_tts is None:
            print("[ANDROID TTS]: NOG NIET GEREED")
            return
        ...
        try:
            ...
            parameters = HashMap()
            uiting_id = JavaString(f"chemie_{int(time.time() * 1000)}")
            java_tekst = JavaString(str(tekst))

            self.android_tts_bezig = True
            print(f"[ANDROID TTS SPREEKT]: {tekst}")

            # Oude speak-signatuur werkt ook op oudere Android-versies.
            resultaat = self.android_tts.speak(
                java_tekst,
                TextToSpeech.QUEUE_FLUSH,
                parameters
            )
            print(f"[ANDROID TTS RESULTAAT]: {resultaat}; ID={uiting_id}")

            # De spreekduur is niet exact beschikbaar via deze oude API.
            # Zet de bezet-vlag na een ruime schatting terug.
            geschatte_duur = max(2.0, min(15.0, len(str(tekst)) / 12.0))
            Clock.schedule_once(self.android_tts_klaar, geschatte_duur)

        except Exception as fout:
            self.android_tts_bezig = False
            print(
                "[ANDROID TTS FOUT BIJ SPREKEN]: "
                f"{type(fout).__name__}: {fout}"
            )

    def android_tts_klaar(self, dt=None):
        self.android_tts_bezig = False
        print("[ANDROID TTS]: SPREEKSESSIE KLAAR")

    @run_on_ui_thread
    def stop_android_tts(self):
        if self.android_tts is None:
            return
        try:
            self.android_tts.stop()
            self.android_tts.shutdown()
        except Exception as fout:
            print(f"[ANDROID TTS STOPFOUT]: {fout}")
        self.android_tts = None
        self.android_tts_listener = None
        self.android_tts_gereed = False
        self.android_tts_initialiseren = False
    ...
    def assistent_spreekt(self, tekst):
        ...
        if platform == "android":
            if not self.android_tts_gereed:
                print(f"[ANDROID TTS]: TEKST IN WACHTRIJ: {tekst}")
                self.wachtende_tts_teksten.append(tekst)
                if not self.android_tts_initialiseren:
                    self.initialiseer_android_tts()
                return

            self.spreek_android_tekst(tekst)
            return
    ...
    def start_android_listener(self, dt=None):
        ...
        try:
            ...
            self.android_luisterfout(
                f"LUISTERSERVICE KON NIET STARTEN: {fout}"
            )
    ...
    def android_luisterstatus(self, status):
        ...
        if status == "MICROFOON ACTIEF - ZEG CHEMI":
            ...
            "SPRAAK VERWERKEN...",
            "MICROFOON HERSTART...",
            "GEEN SPRAAK HERKEND - OPNIEUW LUISTEREN"
        ):
    ...
    def verwerk_android_spraak(self, gesproken_tekst):
        try:
            ...
            # Negeer alleen tekst die volledig ontstaat terwijl TTS spreekt.
            if self.android_tts_bezig:
                print(f"[ANDROID SPRAAK GENEGEERD TIJDENS TTS]: {tekst}")
                return
            ...
            if any(noodzin in tekst for noodzin in noodzinnen):
                huidige_tijd = time.time()
                if huidige_tijd - self.laatste_noodactie < 5:
                    return

                self.laatste_noodactie = huidige_tijd
                self.wacht_op_opdracht = False
                print("[ANDROID ACTIE]: DIRECTE NOODMELDING HERKEND")
                self.verwerk_android_noodgeval(tekst)
                return

            if self.wacht_op_opdracht:
                self.wacht_op_opdracht = False
                print(f"[ANDROID ACTIE]: VERVOLGOPDRACHT: {tekst}")
                self.verwerk_android_opdracht(tekst)
                return

            if "chemi" in tekst or "chemie" in tekst:
                opdracht = tekst.replace("chemie", "", 1)
                opdracht = opdracht.replace("chemi", "", 1)
                opdracht = opdracht.strip(" ,.!?")

                if opdracht:
                    print(f"[ANDROID ACTIE]: DIRECTE OPDRACHT: {opdracht}")
                    self.verwerk_android_opdracht(opdracht)
                    return

                self.wacht_op_opdracht = True
                print("[ANDROID ACTIE]: WAKE-WORD CHEMI HERKEND")
                self.update_ui(
                ...
                Clock.schedule_once(self.reset_wachten_op_opdracht, 12)
                return

            print("[ANDROID ACTIE]: GEEN WAKE-WORD OF NOODMELDING")
            ...
            self.log_status("FOUT BIJ VERWERKEN VAN SPRAAK")
    ...
    def verwerk_android_noodgeval(self, tekst):
        ...
        if stof_id and stof_id in self.lab_database:
            ...
            info = {
                ...
                "pictogram": "",
                "n_ogen": (
                    "Begin direct met het spoelen van de ogen. "
                    "Volg de bestaande noodprocedure en waarschuw direct een collega."
                ),
                "pbm_lab": "",
                "pbm_fabriek": "",
                "pbm_pic_lab": "",
    ...
    def verwerk_android_opdracht(self, tekst):
        ...
        if not stof_id:
            ...
            self.assistent_spreekt("De stof is niet herkend.")
            Clock.schedule_once(
    ...
    def vraag_locatie_en_antwoord(self, info):
        ...
        try:
            ...
            self.assistent_spreekt(
                "Is dit voor het laboratorium of voor de fabriek?"
            )

            begin = time.time()
            while (
                not self.keuze_event.is_set()
                and time.time() - begin < 15
            ):
                time.sleep(0.1)
            ...
            if locatie == "lab":
                locatie_tekst = "laboratorium"
                pbm_tekst = info.get("pbm_lab", "")
                pbm_pictogrammen = info.get("pbm_pic_lab", "")
            else:
                locatie_tekst = "fabriek"
                pbm_tekst = info.get("pbm_fabriek", "")
                pbm_pictogrammen = info.get("pbm_pic_fabriek", "")

            if not pbm_tekst:
                pbm_tekst = "Er zijn geen specifieke beschermingsmiddelen geregistreerd."
            ...
            print(
                ...
                f"locatie={locatie}, "
                f"tekst={pbm_tekst}, "
                f"pictogrammen={pbm_pictogrammen}"
            )
            ...
            self.assistent_spreekt(
                ...
                f"voor het {locatie_tekst} zijn: {pbm_tekst}"
            )

            # Langer zichtbaar voor de test.
            time.sleep(10)
    ...
    def pictogram_pad(self, bestandsnaam):
        bestandsnaam = str(bestandsnaam).strip()
        if not bestandsnaam:
            return ""

        exact_pad = os.path.join(PICTO_DIR, bestandsnaam)
        if os.path.exists(exact_pad):
            return exact_pad

        try:
            gezochte_naam = bestandsnaam.lower()
            for bestaande_naam in os.listdir(PICTO_DIR):
                if bestaande_naam.lower() == gezochte_naam:
                    gevonden_pad = os.path.join(PICTO_DIR, bestaande_naam)
                    print(
                        "[PICTOGRAM GEVONDEN]: "
                        f"{bestandsnaam} -> {bestaande_naam}"
                    )
                    return gevonden_pad
        except Exception as fout:
            print(f"[PICTOGRAM FOUT]: {type(fout).__name__}: {fout}")

        print(f"[PICTOGRAM ONTBREEKT]: {bestandsnaam}")
        return exact_pad
    ...
    def update_ui(self, tekst, hex_bg, hex_txt, pic_string=None):
        def change(dt):
            ...
            if pic_string:
                for naam in [
                    item.strip()
                    for item in str(pic_string).split(",")
                    if item.strip()
                ]:
                    pad = self.pictogram_pad(naam)
                    if pad and os.path.exists(pad):
                        self.pic_layout.add_widget(
                            Image(source=pad, allow_stretch=True)
                        )
                        gevonden_aantal += 1

            if gevonden_aantal > 0:
            ...
            print(
                "[UI BIJGEWERKT]: "
                f"tekst={tekst}, pictogrammen={gevonden_aantal}"
            )
    ...
    def hoofd_loop(self):
        ...
        while True:
            try:
                ...
                tekst = self.recognizer.recognize_google(
                    audio,
                    language="nl-NL"
                ).lower()
    ...
    def start_nood_timer(self, info, minuten=15):
        ...
        instructie = info.get("n_ogen", "") or (
            f"Begin direct met spoelen voor {minuten} minuten."
        )
        self.assistent_spreekt(instructie)
    ...
    def stop_timer_voltooid(self):
        ...
        self.assistent_spreekt(
            "De spoeltijd is voorbij. Controleer het oog."
        )
    ...
    def toon_pdf_in_app(self, pdf_pad):
        ...
        try:
            ...
            self.pdf_paginas_tekst = [
                pagina.extract_text() or "Geen tekst op deze pagina."
        ...
        if not hasattr(self, "pdf_text_label"):
            ...
            self.pdf_text_label.bind(
                width=lambda instantie, breedte:
                    setattr(
                        instantie,
                        "text_size",
                        (max(breedte - 40, 100), None)
                    )
            )
            self.pdf_text_label.bind(
                texture_size=lambda instantie, grootte:
                    setattr(instantie, "height", grootte[1] + 40)
            )
    ...
    def analyseer_msds_pdf(self, pdf_pad):
        """
        Tijdelijke Android-versie.
        De uitgebreide PBM-detectie uit de oude PC-versie wordt
        als volgende stap teruggeplaatst nadat TTS is getest.
        """
        try:
            reader = PdfReader(pdf_pad)
            tekst = "\n".join(
            ...
            for code, pictogram in mapping.items():
                if code in laag:
                    pics.append(pictogram)
            ...
            return {
                ...
                "pictogram": ",".join(pics),
                "n_ogen": (
                    "Bij contact met de ogen, direct spoelen met "
                    "overvloedig water en een arts raadplegen."
                ),
        ...
        if os.path.exists(STOFFEN_CSV):
            try:
                with open(
                    ...
                    bestand.seek(0)
                    scheiding = ";" if ";" in eerste else ","

                    for rij in csv.DictReader(bestand, delimiter=scheiding):
                        schoon = {
                        ...
                        naam = schoon.get(
                            "stof",
                            next(iter(schoon.values()), "")
                        )
                        if naam:
                            database[self.normaliseer_stofnaam(naam)] = {
                                "naam": naam,
                                "pbm_lab": schoon.get("pbm_lab", ""),
                                ...
                                "n_ogen": next(
                                    (
                                        waarde
                                        for sleutel, waarde in schoon.items()
                                        if "ogen" in sleutel
                                    ),
                                    ""
                                ),
                                ...
                                "gevaren": schoon.get(
                                    "gevaren",
                                    "Er zijn geen specifieke gevaren bekend."
                                )
                            }
            except Exception as fout:
            ...
            for naam in os.listdir(MSDS_DIR):
                if naam.lower().endswith(".pdf"):
                    stof_id = self.normaliseer_stofnaam(
                        os.path.splitext(naam)[0]
                    )
                    if stof_id not in database:
                        data = self.analyseer_msds_pdf(
                            os.path.join(MSDS_DIR, naam)
                        )
                        if data:
                            database[stof_id] = data
    ...
    def vind_beste_stof(self, opdracht):
        ...
        for stof_id in ids:
            if stof_id and (
                stof_id in combinatie
                or combinatie in stof_id
            ):
                return stof_id

        stopwoorden = {
            "wat", "zijn", "de", "het", "van", "gevaar", "gevaren",
            "risico", "pbm", "pbms", "bbm", "bbms", "informatie",
            "msds", "pdf", "blad", "laat", "zien"
        }

        for woord in woorden:
            schoon_woord = self.normaliseer_stofnaam(woord)
            if schoon_woord in stopwoorden:
                continue
    ...
    def lees_gevaren_voor(self, info):
        ...
        gevaren_tekst = info.get(
            "gevaren",
            "Er zijn geen specifieke gevaren bekend voor deze stof."
        )
        self.update_ui(
        ...
        self.assistent_spreekt(
            f"De gevaren van {info.get('naam', '')} zijn: {gevaren_tekst}"
        )
        time.sleep(8)
        self.systeem_bezet = False
        self.update_ui("CHEMI", BG_STANDBY, KLEUR_TEKST_DONKER)
    ...
    def open_msds(self, info):
        ...
        if pdf and os.path.exists(pad):
            Clock.schedule_once(
                lambda dt: self.toon_pdf_in_app(pad)
            )
        else:
            self.assistent_spreekt(
